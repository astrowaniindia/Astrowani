...
| `hot-updater.config.ts` — bare + Hermes, supabase db, storage switchable Supabase/R2 (see **CB**), `updateStrategy: 'appVersion'` | identical | identical |
...
- **Now:** the project (`astrowaniindia's Project`, org `astrowaniindia's Org`) is on the
  **Free plan**. The dashboard's Database → Backups page says Free "does not include project
  ...
  - The project also shows **"EXCEEDING USAGE LIMITS"**. Supabase can restrict or pause a
    ...
    - **Update 2026-09-13:** the limit was **storage** (1.82 GB of 1 GB), almost all of it
      old OTA bundles. Cut to 657 MB and new OTAs moved to Cloudflare R2 — see **CB**.
...
## Session 2026-09-13: Supabase storage over quota, OTA bundles moved to Cloudflare R2

### CB. Why Supabase was over quota, and where OTA bundles live now

**The trigger.** On 2026-09-13 Supabase emailed that the org (`oikugtlnwlkeoqsllfaz`, Free
plan) was over quota: **storage 1.82 GB of 1.1 GB**. The Fair Use Policy applies from
**2026-10-13** if it is still over.

- The database was only 25 MB. Almost all of the storage was **hot-updater OTA bundles**:
  158 bundles since August, ~8 MB each, never deleted.
  - `hot-updater-storage-for-astrowani`: 1,143 MB
  - `hot-updater-storage-for-astrowani-vendor`: 555 MB
  - `app-images`: only 31 MB
- Every OTA download also counted against the Free plan's **5 GB/month egress**. At about
  1,000 customers and several deploys a week, that would have become the next limit.

**Cleanup, done 2026-09-13.**
- **124 superseded bundles** were deleted: per bucket + platform + channel + target
  version, the newest 3 were kept, and always the newest enabled one.
- Also deleted: **22 orphan folders** (storage with no `bundles` row) and **56 stale patch
  files**.
- Result: 1,729 MB → **657 MB**, 34 bundles left.
- It was done through the Storage API with the service-role key, not SQL. Deleting
  `storage.objects` rows does not delete the files.
- `bundle_patches` has `ON DELETE CASCADE` on `base_bundle_id`. Deleting an old base only
  means phones still on it download the full bundle instead of a diff. Nothing breaks.
- The dashboard kept showing 1.77 GB for a while afterwards. Supabase storage usage refreshes
  roughly daily; the database figure was already correct.

**The move to R2** (commit `16e08e3`). Cloudflare account `3d7992c44239142cf889c827e26d551f`.
R2 has free egress and 10 GB of free storage.

- **Buckets:** `astrowani-ota-customer` and `astrowani-ota-vendor`. One R2 API token
  (Object Read & Write, both buckets only).
- **Upload side:** `hot-updater.config.ts` in both apps uses `s3Storage` from
  `@hot-updater/aws` (exact 0.35.10) when **`HOT_UPDATER_STORAGE=r2`** in
  `.env.hotupdater`, otherwise `supabaseStorage`. The same file holds
  `HOT_UPDATER_R2_ENDPOINT`, `_BUCKET_NAME`, `_ACCESS_KEY_ID` and `_SECRET_ACCESS_KEY`. It
  is git-ignored.
- **As of 2026-09-13 both apps are set to `r2`.**
- **Serving side:** installed apps call
  `https://fxpoustnddrgumhwdcma.supabase.co/functions/v1/update-server` by URL. That is baked
  into the binaries, so the function stayed where it is.
  - It now reads from **two storages**: `supabase-storage://` URIs from Supabase, and
    `s3://` URIs from R2 via presigned links.
  - **No store build was needed.**
- **The function source is now in the repo:**
  `astrowani_vendors-main/supabase-functions/update-server/`. Before this it existed only
  in a temp dir generated by the hot-updater CLI at deploy time.
  - `r2Storage.ts` is a small aws4fetch runtime storage.
  - `s3Storage` was not reused because its runtime refuses any bucket but its own, and this
    one function serves both apps' buckets.
  - It needs Edge Function secrets `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` and
    `R2_ENDPOINT`. Without them it logs a warning and serves Supabase bundles only.
  - `GET /update-server/storages` returns `{"r2":true}` when R2 is wired.
- **Deploying the function:** Supabase MCP `deploy_edge_function` or the Supabase CLI, with
  `verify_jwt: false`. **Do not** regenerate it with `hot-updater init`: that overwrites it
  with the Supabase-only template, and every R2 bundle then fails its update check.
- **Verified before switching:** the new function returned identical responses (signed
  tokens aside) to the old one for customer and vendor, android and ios, on six target
  versions.
- **Verified on-device:** first R2 bundles `01a0996e` (android) and `01a09972` (ios). The
  download host was `*.r2.cloudflarestorage.com` and the SHA-256 matched `fileHash`. The
  change arrived on a real Android phone.
- **What Supabase still carries per launch:** one edge-function update check. That is ~30
  bytes when up to date, 1–17 KB when an update exists. At 1,000 customers × 5 launches a
  day: ~150k invocations a month (limit 500k) and well under 100 MB of egress.

> **Two traps hit while shipping this:**
> - **`npm run deploy:ota -- -m "two words"` breaks on Windows.** npm drops the quotes, and
>   `deployOta.js` spawns with `shell: true`, so the message is split. hot-updater rejects
>   the extra arguments and deploys nothing ("too many arguments for 'deploy'"). Call the
>   script directly and quote the value twice:
>   `node scripts/deployOta.js -m '"message with spaces"'`.
> - **The first R2 deploy of each app creates no bsdiff patches.** Its log shows "No storage
>   plugin for protocol: supabase-storage", because the CLI's config has only the R2 storage
>   and cannot read the previous (Supabase) bundle. Phones download the full ~6 MB once.
>   Later R2 → R2 deploys patch normally.

**Also shipped this session (customer app, OTA from R2):**
- `d044bd4`: the Home astrologer rows can be dragged (`useDraggableMarquee`).
- `49003ce`: **drawer swipe-to-open removed** (`swipeEnabled: false`); the hamburger icon
  opens it. Its native swipe pan over the left third of the screen beat the cards' JS drag,
  so dragging the cards right opened the sidebar. The cards were judged more important.

### CC. Still open from CB, with dates

- **Vendor app: no R2 bundle yet.** Its `.env.hotupdater` is set to `r2` (write to the
  bucket tested 2026-09-13), but nothing has been deployed since. Its phones still download
  from Supabase until the next vendor `npm run deploy:ota`.
- **626 MB of old bundles are still in Supabase Storage** (the 34 kept on 2026-09-13).
  - **When:** after the vendor app's first R2 deploy.
  - **What:** delete every Supabase bundle that an R2 bundle has superseded for the same
    bucket + platform + channel + target version.
  - **Keep:** anything still newest for a target version that has no R2 bundle yet. Phones
    on older store builds (e.g. customer `24.0.x`/`24.0`, vendor `6.5.x`/`6.5`) still
    download those.
  - **Expected end state:** Supabase Storage ≈ `app-images` only (~31 MB).
  - **How:** the Storage API with the service-role key, never SQL deletes (see CB).
...
- **Old OTA bundles in R2 will pile up the same way.** R2 is 10 GB free, not unlimited.
  ...
  per target" rule).
- **Node 22 before 2027-01.** The AWS SDK that `@hot-updater/aws` pulls in drops Node 20
  support for releases after the first week of January 2027. The dev machine runs Node
