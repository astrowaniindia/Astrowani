...
const HomeScreen = () => {
  ...
  // Popup state — a QUEUE, not a single value. A single popupData object used to mean a
  // second incoming request (different customer, or a socket/Realtime race for the same
  // one) silently overwrote/dropped the first — that request's row was left pending until
  ...
  const enqueuePopup = (item) => {
    setPopupQueue((prev) => {
      const exists = prev.some(
        (p) =>
          (item.requestId && p.requestId === item.requestId) ||
