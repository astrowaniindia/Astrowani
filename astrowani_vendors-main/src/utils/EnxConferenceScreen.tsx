  ...
  async requestPermission() {
    try {
      ...
      if (granted === PermissionsAndroid.RESULTS.GRANTED) {
        ...
        Alert.alert('Kindly grant storage permission to send logs.');
      }
    } catch (err) {
      console.warn(err);
    }
        ...
        <View style={{flex: 1}} onLayout={this._onLayout.bind(this)}>
          ...
          {this.state.isConnected && (
            <View style={styles.receiverImageContainer}>
              <Image source={{uri: this.props.route.params?.receiverImage || 'https://via.placeholder.com/100'}} style={styles.receiverImage} />
            </View>
          )}
