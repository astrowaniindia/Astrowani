  ...
  checkAndroidPermissions = async (): Promise<void> => {
    try {
      ...
      if (deniedPermissions.length > 0) {
        throw new Error(`Permissions denied: ${deniedPermissions.map(([key]) => key).join(', ')}`,);
      }
    } catch (error) {
  ...
  getRoomTokenWebCall = async (): Promise<string | null> => {
    ...
    try {
      const response = await axios.post(
        `https://astrowani-fb6pi.ondigitalocean.app/api/call/initiate`,
        {
          receiverId: userId,
          callType: "voice",
          callerRole: callerRole
  ...
  navigateToVideo = async (token: any): Promise<void> => {
    ...
    if (resolvedCallType === 'video') {
      this.props.navigation.navigate('VoiceCallScreen', {
        ...
        roomID,
        userId,
        receiverImage: 'https://via.placeholder.com/100', // Default image if not available
