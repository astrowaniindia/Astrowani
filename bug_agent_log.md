...
|---|---|---|---|---|
| 2026-08-05 | `TypeError: Cannot read property 'map' of undefined` in `formatAuspiciousTime` (Sentry `REACT-NATIVE-1`, https://astrowani.sentry.io/issues/7653834455/) — `PanchangScreen.js` crashed because the backend's `/api/free-services/panchang` never returns `auspicious_period`/`inauspicious_period` | customer | fix-merged | PR #1 (https://github.com/astrowaniindia/Astrowani/pull/1) merged 2026-08-05T17:45:30Z — defaults both fields to `[]` before `.map()`. Duplicate PR #2 (`fix/panchang-screen-crash`) was closed unmerged, but its extra hardening is on `main` anyway (verified 2026-08-16: `PanchangScreen.js:342`/`:352` have `Array.isArray` guards, and `formatPanchangData`'s `nakshatra[0]`/`tithi[0]`/`karana[0]`/`yoga[0]` indexing is optional-chained) — landed via a manual cherry-pick outside the PR flow. **The underlying data gap is now closed too (2026-08-16)**: the endpoint had only ever returned 5 of the fields the screen reads, so the two period sections were permanently empty (that emptiness is what the original `.map()` crash was) and "Additional Info" rendered four `Invalid Date` rows. The Jyotisham field names the earlier runs couldn't verify were probed live and are: `advanced_details.abhijitMuhurta.{start,end}` and `advanced_details.{sun_rise,sun_set,moon_rise,moon_set}` (bare clock times, no date), plus `rahukaal`/`gulika`/`yamakanta` ("H:MM AM to|- H:MM PM") and `bhadrakaal` (a full datetime range, since Bhadra follows the Vishti karana and can cross midnight). All are now mapped, anchored to the response's own date and emitted as ISO-local. |
